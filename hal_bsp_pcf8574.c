...
void set_water(uint8_t status)  //实际是水泵
{
    if (status == true) {
        tmp_io.bit.p1 = 1; // 水泵开启
        PCF8574_Write(tmp_io.all);
    } else if (status == false) {
        tmp_io.bit.p1 = 0; // 水泵关闭
        PCF8574_Write(tmp_io.all);
    }
}

void set_buzzer(uint8_t status)  //实际是提示施肥
{
    if (status == true) {
        tmp_io.bit.p2 = 0; // 蜂鸣器开启
        PCF8574_Write(tmp_io.all);
    } else if (status == false) {
        tmp_io.bit.p2 = 1; // 蜂鸣器关闭
        PCF8574_Write(tmp_io.all);
    }
}

void set_sunshade(uint8_t status)  //实际是提示遮阳
{
    if (status == true) {
        tmp_io.bit.p3 = 1; // LED开启
        PCF8574_Write(tmp_io.all);
    } else if (status == false) {
        tmp_io.bit.p3 = 0; // LED关闭
        PCF8574_Write(tmp_io.all);
    }
}
